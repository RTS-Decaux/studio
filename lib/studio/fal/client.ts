...
export class FalClient {
  ...
  async getResult(
    ...
    try {
      const result: FalResult<FalGenerationOutput> = await fal.queue.result(
        modelId,
        { requestId }
      );
  ...
  private normalizeStatusString(status: string): FalGenerationStatus {
    ...
    if (upperStatus.includes("QUEUE")) {
      return "IN_QUEUE";
    }
    if (upperStatus.includes("PROGRESS")) {
      return "IN_PROGRESS";
    }
  ...
  private createError(
    ...
    const FalClientError = (require("./types") as typeof import("./types"))
      .FalClientError;
